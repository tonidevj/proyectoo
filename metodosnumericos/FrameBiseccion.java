...
// Define el paquete donde se encuentra la clase. El paquete "metodosnumericos" es parte de la estructura de organización del proyecto.
import java.math.BigDecimal;
// Importa la clase BigDecimal del paquete java.math. BigDecimal se utiliza para realizar cálculos de precisión arbitraria, especialmente en operaciones financieras donde la precisión es crítica.
import java.math.RoundingMode;
// Importa la clase RoundingMode del paquete java.math. RoundingMode se utiliza para especificar el comportamiento de redondeo en operaciones con BigDecimal.
import java.util.ArrayList;
// Importa la clase ArrayList del paquete java.util. ArrayList es una implementación de la interfaz List que permite almacenar elementos de manera dinámica.
import javax.swing.JOptionPane;
// Importa la clase JOptionPane del paquete javax.swing. JOptionPane se utiliza para mostrar cuadros de diálogo estándar, como mensajes de alerta, cuadros de texto y opciones.
import javax.swing.JTable;
// Importa la clase JTable del paquete javax.swing. JTable se utiliza para crear y gestionar tablas en una interfaz gráfica de usuario (GUI).
import javax.swing.table.DefaultTableModel;
// Importa la clase DefaultTableModel del paquete javax.swing.table. DefaultTableModel es una implementación de TableModel que gestiona los datos y la estructura de las tablas en una JTable.
import javax.swing.table.TableColumnModel;
// Importa la clase TableColumnModel del paquete javax.swing.table. TableColumnModel gestiona la estructura y el comportamiento de las columnas en una JTable.
import metodosnumericos.interfaces.Funcion;
// Importa la interfaz Funcion que está en el paquete metodosnumericos.interfaces. Esto indica que el código utiliza o implementa funcionalidades definidas en esta interfaz.
import metodosnumericos.interfaces.Ifuntion;
// Importa la interfaz Ifuntion que está en el paquete metodosnumericos.interfaces. Esto también sugiere que el código implementará o utilizará las funcionalidades definidas en esta interfaz.
import org.lsmp.djep.djep.DJep;
// Importa la clase DJep del paquete org.lsmp.djep.djep. DJep es una biblioteca que se utiliza para el análisis y manipulación de expresiones matemáticas (como derivadas, simplificaciones, etc.).
import org.nfunk.jep.Node;
// Importa la clase Node del paquete org.nfunk.jep. Node representa un nodo en el árbol sintáctico que DJep genera cuando analiza una expresión matemática.
import org.nfunk.jep.ParseException;
// Importa la clase ParseException del paquete org.nfunk.jep. ParseException se utiliza para manejar las excepciones que se producen cuando DJep no puede analizar una expresión matemática correctamente.


public class FrameBiseccion extends javax.swing.JFrame {

    // Declaración de variables
    Ifuntion f; // Interfaz para funciones matemáticas
    Funcion fun; // Función específica que se va a utilizar
    DefaultTableModel modelo; // Modelo de tabla para mostrar datos en una JTable
    byte controlador = 0; // Variable de control para los paneles
    Grafica gr; // Objeto para graficar las funciones

    // Constructor de la clase FrameBiseccion
    public FrameBiseccion() {
        initComponents(); // Inicializa los componentes gráficos
        this.setSize(1000, 550); // Establece el tamaño de la ventana
        this.setLocationRelativeTo(null); // Centra la ventana en la pantalla
        lock(controlador); // Llama al método lock para configurar el panel visible
        ordenar(); // Ordena elementos si es necesario (suposición por el nombre)
    }

    // Método para bloquear o desbloquear paneles en función de la posición
    public void lock(byte posicion) {
        switch (posicion) {
            case 0:
                // Muestra el panel de Bisección y oculta los demás
                panelNewton.setVisible(false);
                panelSecante.setVisible(false);
                panelBi.setVisible(true);
                GrupoPn.setSelectedIndex(0); // Selecciona la pestaña correspondiente
                controlador = 0; // Actualiza el controlador a la posición actual
                break;
            case 1:
                // Muestra el panel de Newton y oculta los demás
                panelBi.setVisible(false);
                panelSecante.setVisible(false);
                panelNewton.setVisible(true);
                GrupoPn.setSelectedIndex(1); // Selecciona la pestaña correspondiente
                controlador = 1; // Actualiza el controlador a la posición actual
                break;
            case 2:
                // Muestra el panel de la Secante y oculta los demás
                panelBi.setVisible(false);
                panelNewton.setVisible(false);
                panelSecante.setVisible(true);
                GrupoPn.setSelectedIndex(2); // Selecciona la pestaña correspondiente
                controlador = 2; // Actualiza el controlador a la posición actual
                break;
        }
    }
    ...
    private void jButton23ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton23ActionPerformed
        // Método que se ejecuta cuando se presiona el botón asociado a jButton23

        limpiarTabla(); // Limpia los datos de la tabla de resultados
        modelo = (DefaultTableModel) tbResultado.getModel(); // Obtiene el modelo de la tabla de resultados
        String funcion = pantalla.getText(); // Obtiene la función ingresada en la pantalla
        fun = new Funcion(funcion); // Crea un objeto Funcion con la cadena de la función

        // Verifica si la función no está vacía y contiene la variable 'x'
        if ((!funcion.equals("")) && fun.contieneX(funcion)) {

            // Controla el flujo dependiendo del método seleccionado (bisección, Newton o secante)
            if (controlador == 0) { // Si el método seleccionado es bisección (controlador = 0)

                try {
                    // Intenta obtener los valores de a, b y t (intervalo y tolerancia) desde los campos de texto
                    double a = Double.parseDouble(txtA.getText());
                    double b = Double.parseDouble(txtB.getText());
                    double t = Double.parseDouble(txtT.getText());
                    this.f = fun; // Asigna la función a la variable 'f'
                    metodoBiseccion(funcion, a, b, t); // Llama al método de bisección
                    ordenar(); // Ordena los resultados (suposición basada en el nombre)
                } catch (NumberFormatException e) {
                    // Captura el error si los valores de entrada no son números válidos
                    JOptionPane.showMessageDialog(null, "Verifique que el intervalo o tolerancia sea correcto.", "Error", JOptionPane.ERROR_MESSAGE);
                }

            } else if (controlador == 1) { // Si el método seleccionado es Newton-Raphson (controlador = 1)

                try {
                    // Deriva la función para usarla en el método de Newton-Raphson
                    Ifuntion der;
                    String derivada = Derivar(funcion); // Deriva la función ingresada
                    Funcion fun2 = new Funcion(derivada); // Crea un objeto Funcion para la derivada
                    double iter = Double.parseDouble(txtIter.getText()); // Obtiene el número de iteraciones desde el campo de texto
                    double ini = Double.parseDouble(txtIni.getText()); // Obtiene el valor inicial desde el campo de texto
                    double error = Double.parseDouble(txtTolerancia.getText()); // Obtiene la tolerancia desde el campo de texto
                    this.f = fun; // Asigna la función original a 'f'
                    der = fun2; // Asigna la función derivada a 'der'

                    // Llama al método de Newton-Raphson con la función original, la derivada, el valor inicial, la tolerancia y el número de iteraciones
            ...
            } else if (controlador == 2) { // Si el método seleccionado es la secante (controlador = 2)
                try {
                    // Intenta obtener los valores de x0, x1, iteraciones y tolerancia desde los campos de texto
                    ...
                    this.f = fun; // Asigna la función a 'f'

                    // Llama al método de la secante con los valores obtenidos
            ...
            // Muestra un mensaje de error si no se ingresó una función válida con la variable 'x'
